...
// 손패 · 화투 패처럼 화면 아래에서 솟는 부채. 회전축이 카드 바로 아래라 양끝이 손에 쥔 것처럼 눕는다.
export const FAN = {
  // 19장이었다가 11장으로 줄였다. 손패는 한 장이 카드로 읽혀야 한다 —
  // 한 장당 드러나는 폭이 16px(19장)이면 줄무늬, 29px(11장)이면 패다. 13장(25px)도 아직 빽빽했다
  count: 11,
  get mid() {
    return (this.count - 1) / 2
  },
  // 장수를 늘릴 때는 간격을 같이 좁혀 양끝 각도(~47°)를 지킨다.
  // 간격을 두고 장수만 늘리면 부채가 넓어지고 양끝이 눕는다
  step: 9.4, // 각도 간격. 카드 크기가 달라져도 간격 비율은 유지된다
  origin: '50% 165%', // 회전축. 카드 높이의 1.65배 지점 — 손목쯤이다
  lift: 40, // 최대 들림
